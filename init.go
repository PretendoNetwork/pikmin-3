...
func init() {
	...
	accountGRPCHost := os.Getenv("PN_PIKMIN3_ACCOUNT_GRPC_HOST")
	accountGRPCPort := os.Getenv("PN_PIKMIN3_ACCOUNT_GRPC_PORT")
	accountGRPCAPIKey := os.Getenv("PN_PIKMIN3_ACCOUNT_GRPC_API_KEY")

	if strings.TrimSpace(postgresURI) == "" {
		globals.Logger.Error("PN_PIKMIN3_POSTGRES_URI environment variable not set")
		os.Exit(0)
	}
	...
	if port, err := strconv.Atoi(authenticationServerPort); err != nil {
		globals.Logger.Errorf("PN_PIKMIN3_AUTHENTICATION_SERVER_PORT is not a valid port. Expected 0-65535, got %s", authenticationServerPort)
		os.Exit(0)
	...
	if strings.TrimSpace(secureServerHost) == "" {
		globals.Logger.Error("PN_PIKMIN3_SECURE_SERVER_HOST environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(secureServerPort) == "" {
		globals.Logger.Error("PN_PIKMIN3_SECURE_SERVER_PORT environment variable not set")
		os.Exit(0)
	}

	if port, err := strconv.Atoi(secureServerPort); err != nil {
		globals.Logger.Errorf("PN_PIKMIN3_SECURE_SERVER_PORT is not a valid port. Expected 0-65535, got %s", secureServerPort)
		os.Exit(0)
	} else if port < 0 || port > 65535 {
		globals.Logger.Errorf("PN_PIKMIN3_SECURE_SERVER_PORT is not a valid port. Expected 0-65535, got %s", secureServerPort)
		os.Exit(0)
	...
	if strings.TrimSpace(accountGRPCHost) == "" {
		globals.Logger.Error("PN_PIKMIN3_ACCOUNT_GRPC_HOST environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(accountGRPCPort) == "" {
		globals.Logger.Error("PN_PIKMIN3_ACCOUNT_GRPC_PORT environment variable not set")
		os.Exit(0)
	}

	if port, err := strconv.Atoi(accountGRPCPort); err != nil {
		globals.Logger.Errorf("PN_PIKMIN3_ACCOUNT_GRPC_PORT is not a valid port. Expected 0-65535, got %s", accountGRPCPort)
		os.Exit(0)
	} else if port < 0 || port > 65535 {
		globals.Logger.Errorf("PN_PIKMIN3_ACCOUNT_GRPC_PORT is not a valid port. Expected 0-65535, got %s", accountGRPCPort)
		os.Exit(0)
	...
	if strings.TrimSpace(accountGRPCAPIKey) == "" {
		globals.Logger.Warning("Insecure gRPC server detected. PN_PIKMIN3_ACCOUNT_GRPC_API_KEY environment variable not set")
	}
